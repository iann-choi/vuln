...
|No.|기능|
|---|---|
|1|BlackDuck SCA 결과에서 취약 컴포넌트 목록 추출|
|2|NVD, CISA KEV, OSV, Github Advisory DB, BlackDuck Advisory 5개 소스 교차 분석|
|3|Claude AI(claude-sonnet-4-6)를 활용한 한국어 취약점 분석 및 조치 방안 생성|
|4|Canvas 문서 형식으로 리포트 전송(Slack)|
...
```
1. BlackDuck API Token 발급
   1) BlackDuck 웹 UI에 로그인
   2) 우측 상단 사용자 아이콘 > Access Tokens
   3) ‘Create Token’ 버튼 클릭 후 토큰 생성
   4) 생성된 토큰을 `BLACKDUCK_API_TOKEN`에 설정

2. Anthropic API Key

3. Slack Bot Token / Signing Secret 확인
   1) https://api.slack.com/apps 접속 후 ‘is-bot’ 선택
   2) Install App 메뉴 클릭 후 Bot User OAuth Token(`xoxb-...`) 값을 `SLACK_BOT_TOKEN`에 설정
   3) Basic Information > App Credentials > Signing Secret을 `SLACK_SIGNING_SECRET`에 설정

4. Slack Channel ID 확인
   1) Slack 앱에서 리포트를 받을 채널 우클릭 > 채널 세부 정보 열기
   2) 하단에 표시되는 채널 ID(`C`로 시작)를 `SLACK_CHANNEL_ID`에 설정

5. Slack Canvas ID 확인
   1) `F0AMANRNA02`

6. NVD API Key 발급
   1) https://nvd.nist.gov/developers/request-an-api-key 접속
...
7. Github Personal Access Token 발급
   ...
   3) Scopes에서 `public_repo` (또는 `read:packages`) 선택 후
   4) 생성된 토큰(`ghp_...`)을 `GITHUB_TOKEN`에 설정

### 3. Python 환경 구성
1. 파이썬 패키지 관리 도구(uv) 설치
